...
export const METHODOLOGY: Record<string, Entry> = {
  ...
  'selection-composite': {
    ...
    body: [
      ...
      { kind: 'text', text: 'Stage two takes its macro terms from the Macro surface\u2019s shortlist, verbatim. The transformation library was already swept, filtered and narrowed by a person there; the search\u2019s job is combinations, not rediscovery. Each shortlisted term is screened once against each core (fitted sign against the economic prior, at a loose cutoff, with removals reported by reason), and the survivors enumerate in every combination of one to three: at most one term per underlying economic series, where a base variable and its derived forms, unemployment and its year over year change say, are one family; never one series at two lags; no pair beyond the correlation cap. Each core plus combination is refitted jointly, and core coefficients that flip sign or shift materially are flagged.' },
