...
export default function MacroPanel({ portfolio }: { portfolio: string }) {
  ...
  const spliceOption = useMemo(() => {
    ...
    return {
      ...baseOption(),
      grid: { left: 56, right: 18, top: 14, bottom: 30 },
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card>
          <CardHead title={`${d?.label ?? baseKey} — history and scenario paths`}
            ...
            caption="Real FRED history joined to the Federal Reserve's published forward paths. The seam is annotated, not hidden: solid to the left, dashed to the right."
        ...
        <Card>
          <CardHead title="Frequency reconciliation"
            ...
            caption="The conversion shown rather than buried. Published points over the derived monthly series."
      ...
      {d && (
        <Card>
          <CardHead title="Metadata contract"
            caption="Every frequency conversion is driven by this table, never by a global rule." />
